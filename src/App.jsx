...
export default function PastelFitApp() {
  ...
  const [viewingUserId, setViewingUserId] = useState(null); // สำหรับ Coach ดูข้อมูล User
  const [loginError, setLoginError] = useState(''); // เพิ่ม state เก็บข้อความแจ้งเตือนตอนล็อคอิน
  ...
  useEffect(() => {
    // ระบบเคลียร์รูปภาพที่เก่ากว่า 7 วัน (ตามเงื่อนไขข้อ 9)
    const cleanOldImages = () => {
      ...
      setFoodLogs(prev => prev.map(log => {
        ...
        if (log.imageUrl && logDate < sevenDaysAgo) {
          // ในระบบจริง จะเรียก API ไปลบรูปที่ Storage
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-pink-50 pb-20 font-sans">
        ...
        {/* Public Dashboard (Community Status) */}
           ...
           <div className="mt-20 bg-white p-8 rounded-3xl shadow-xl max-w-md mx-auto text-center border border-pink-100">
             ...
             <form onSubmit={handleLogin} className="space-y-4">
               <select name="userId" className="w-full p-4 bg-gray-50 rounded-2xl outline-none focus:ring-2 focus:ring-pink-300 border border-gray-200 text-gray-700 font-medium">
                 ...
                 {users.map(u => (
                   <option key={u.id} value={u.id}>{u.name} ({u.role === 'coach' ? 'โค้ช' : 'สมาชิก'})</option>
                 ))}
               </select>
               <input 
                 type="password" 
                 name="password" 
                 placeholder="รหัสผ่าน (User = 123, Coach = admin)" 
    ...
    <nav className="bg-white shadow-sm fixed w-full top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 flex justify-between items-center h-16">
        ...
        <div className="flex items-center gap-4">
          {currentUser.role === 'coach' && activeTab === 'coach-user-detail' && (
  ...
  const TDEECalculator = ({ targetUserId }) => {
    ...
    const safeTdee = currentData.tdee || 0; // ป้องกันค่า undefined
    ...
    const calculateTDEE = () => {
      ...
      // UI feedback simulation without alert
    ...
    // Calculate on mount if not calculated
    useEffect(() => {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Results Panel based on Reference Image */}
...
Ensure the response is a pure JSON string.`;
        ...
        const apiKey = ""; // API Key จะถูกจัดการโดยระบบ
  ...
  const Dashboard = ({ targetUserId }) => {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* Weekly Sunday Check-in Warning */}
  ...
  const MeasurementTracker = ({ targetUserId }) => {
    ...
    const saveMeasurement = () => {
      if(!form.weight || !form.waist || isReadOnly) return; // Simple validation
  ...
  const RankingBoard = () => {
    // Calculate score dynamically for all users based on real data
    const allUsersScores = users.filter(u => u.role === 'user').map(u => {
      ...
      if(consumed === 0) score = 0; // 0 if no entries today
  ...
  const CoachDashboard = () => {
    ...
    const [coachTab, setCoachTab] = useState('daily-summary'); // 'daily-summary' | 'members'
    const [summaryDate, setSummaryDate] = useState(getTodayString());
    const [filterStatus, setFilterStatus] = useState('all'); // 'all', 'ontrack', 'over', 'nolog'
    const [coachNotes, setCoachNotes] = useState({}); // { userId_date: note }
    ...
    // Compute Daily Summary Metrics across all members for selected summaryDate
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-gradient-to-r from-blue-600 to-indigo-700 p-6 rounded-3xl text-white shadow-xl gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="bg-blue-400/30 text-blue-100 text-xs px-3 py-1 rounded-full font-semibold border border-blue-300/30">
                🔒 สิทธิ์พิเศษเฉพาะ Coach เท่านั้น
              </span>
            </div>
          ...
          <div className="flex items-center gap-2 bg-white/10 backdrop-blur-md p-1.5 rounded-2xl border border-white/20 self-stretch md:self-auto">
            ...
            >
              👥 จัดการสมาชิก ({memberUsers.length})
            </button>
          </div>
        </div>

        {/* TAB 1: DAILY OVERVIEW SUMMARY */}
        {coachTab === 'daily-summary' && (
          <div className="space-y-6 animate-fadeIn">
            {/* Top Bar: Date Selector & Quick Filters */}
        ...
        {coachTab === 'members' && (
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-blue-100">
              <h3 className="text-lg font-bold text-gray-800 mb-4">✨ เพิ่มสมาชิกใหม่</h3>
              <form onSubmit={handleAddUser} className="flex flex-col md:flex-row gap-4 items-end">
                <div className="flex-1 w-full">
                  <label className="block text-sm text-gray-600 mb-1">ชื่อสมาชิก</label>
                  <input 
                    type="text" 
                    value={newUserForm.name}
                    onChange={e => setNewUserForm({...newUserForm, name: e.target.value})}
                    placeholder="เช่น น้องแจน"
                    className="w-full p-3 bg-gray-50 rounded-xl outline-none focus:ring-2 focus:ring-blue-300 border border-gray-200 text-sm"
                  />
                </div>
                <div className="flex-1 w-full">
                  <label className="block text-sm text-gray-600 mb-1">ตั้งรหัสผ่านสำหรับเข้าสู่ระบบ</label>
                  <input 
                    type="text" 
            ...
            <h3 className="text-xl font-bold text-gray-800 pt-2">👥 รายชื่อสมาชิกทั้งหมด</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div key={user.id} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 hover:shadow-md transition">
                    ...
                    {confirmDeleteId === user.id ? (
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => setConfirmDeleteId(user.id)} className="flex-1 bg-red-50 text-red-500 py-2 rounded-xl text-sm font-medium hover:bg-red-100 transition">ลบ</button>
                      </div>
                    )}
                  </div>
                )
